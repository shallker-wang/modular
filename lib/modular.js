var fs = require('fs'),
    ...
    error = require('../util/debug').error('Modular');
...
Modular.writeBuild = function(content, successListener) {
  if (typeof successListener === 'undefined') {
    successListener = function() {}
  }
  if (Modular.option.compress) {
    content = Modular.compress(content);
  }
  var file = Modular.option.output + Modular.option.file;
