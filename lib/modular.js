...
Modular.watch = function(path) {
  path = path.replace(/\/$/, '') + '/';
  fs.watch(path, {interval: 1000}, Modular.onWatch);

  // deep watch
  var files = fs.readdirSync(path);
  files.forEach(function(file, index) {
    if (fs.statSync(path + file).isDirectory()) {
      Modular.watch(path + file);
    }
  })
}

Modular.onWatch = function(ev, fileName) {
  log('watch', ev, fileName);
  var ext = fileName.split('.').pop();
  if (Modular.option.extensions.indexOf(ext) === -1) {
    return;
  }
  if (fileName === Modular.option.file) {
    return;
  }
  Modular.build();
}
...
Modular.writePackage = function(file, content) {
